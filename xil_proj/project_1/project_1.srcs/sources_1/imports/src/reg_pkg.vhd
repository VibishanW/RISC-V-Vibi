...
package reg_pkg is
  ...
  constant C_WIDTH : integer := 32 - 1;
  
  type t_reg_file is array(0 to 31) of STD_LOGIC_VECTOR(C_WIDTH downto 0);
  type t_data_mem is array(0 to 1023) of STD_LOGIC_VECTOR(C_WIDTH downto 0);
  
  constant C_REG_FILE_INIT : t_reg_file := (
    others => (others => '0')
    );
    
  constant C_DATA_MEM_INIT : t_data_mem := (
    others => (others => '0')
    );
